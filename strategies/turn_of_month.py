"""Strategy implementing the Turn of Month trading logic on financial data."""
...
class TurnOfMonthStrategy(StrategyBase):
    """Strategy that trades based on the turn of the month effect."""

    def run(self, data, initial_investment, **kwargs):
        """
        Run the Turn of Month strategy backtest.
        ...
        trade_dates = [
            first_date
        ]  # Start with the first date for alignment with other strategies
    ...
    def get_month_boundaries(self, data):
        """
        Identify the trading boundaries at month transitions.

        For each month transition, find the 5th-to-last day of the current month
        and the 3rd day of the next month for entry and exit.

        Parameters:
        - data: DataFrame with price data

        Returns:
        - List of tuples with (entry_date, exit_date)
        """
