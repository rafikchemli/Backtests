...
def get_cached_data(symbols, start_date, end_date, data_cache, interval="1d", auto_adjust=True):
    """
    ...
    if isinstance(symbols, str):
        ...
        if key not in data_cache:
            data_cache[key] = download_data(symbols, start_date, end_date, interval, auto_adjust)
        ...
        combined_data = download_data(symbols, start_date, end_date, interval, auto_adjust)
        
        # Cache individual symbols and prepare return data
        result_data = {}
        for s in symbols:
            key = f"{s}_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}"
            if isinstance(combined_data, pd.DataFrame) and isinstance(combined_data.columns, pd.MultiIndex):
...
def calculate_date_range(years_back):
    ...
    # Add 1 extra year to ensure we have enough data for calculations (e.g., moving averages)
...
def load_strategy(strategy_key, strategies_config):
    ...
    module_path, class_name, symbol, years_back, initial_investment = strategies_config[strategy_key]
...
def structure_equity_curve(trade_dates, portfolio_curve, initial_investment, data=None, bh_symbol=None, benchmark_data=None, standard_benchmark=None):
    """
    ...
    if standard_benchmark is not None:
        ...
        for date in x_dates:
            ...
            if date in standard_benchmark:
                ...
                closest_dates = sorted(standard_benchmark.keys(), key=lambda d: abs((d - date).total_seconds()))
                if closest_dates:
...
def print_results(strategy_name, equity, bh_equity, n_trades, avg_gain_pct, periods_label="trade", standard_benchmark=None):
    """
