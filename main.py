"""Main module for running backtest strategies on financial data."""

import argparse

from core.backtest_utils import calculate_date_range, get_cached_data, load_strategy
...
# {name: (module_path, class_name, symbol, years_back, initial_investment)}
STRATEGIES = {
    "ma200": ("strategies.ma200", "MA200Strategy", "SPY", 20, 10000),
    "monday_reversal": (
        "strategies.monday_reversal",
        "MondayReversalStrategy",
        "SPY",
        20,
        10000,
    ),
    "overnight_swing": (
        "strategies.overnight_swing",
        "OvernightSwingStrategy",
        "SPY",
        20,
        10000,
    ),
    ...
    "spy_tlt_rotation": (
        "strategies.spy_tlt_rotation",
        "SpyTltRotationStrategy",
        ["SPY", "TLT"],
        20,
        10000,
    ),
    "turn_of_month": (
        "strategies.turn_of_month",
        "TurnOfMonthStrategy",
        "SPY",
        20,
        10000,
    ),
    "berkshire": ("strategies.berkshire", "BerkshireStrategy", "BRK-B", 20, 10000),
}
...
def prepare_benchmark(start_date, end_date, initial_investment):
    ...
    benchmark_data = get_cached_data(
        benchmark_symbol, start_date, end_date, BENCHMARK_DATA
    )
...
def run_strategy(strategy_key):
    ...
    try:
        ...
        strategy_class, symbol, years_back, initial_investment = load_strategy(
        ...
        benchmark_data, standard_benchmark = prepare_benchmark(
...
def main():
    """Parse command line arguments and run specified strategies."""
    parser = argparse.ArgumentParser(description="Run backtest strategies.")
    parser.add_argument(
        "--strategy", type=str, choices=STRATEGIES.keys(), help="Strategy to run"
    )
